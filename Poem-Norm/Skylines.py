...
class Skylines:
    ...
    def parallelValidate(self, param, reportValidationResult):
        predictionError, classifier, clippedDiagnostic, unclippedDiagnostic = self.generateModel(param, reportValidationResult)
        return {'param': param, 'classifier': classifier, 'perf': predictionError, 'clippedDiagnostic': clippedDiagnostic, 'unclippedDiagnostic': unclippedDiagnostic}
    ...
    def validate(self):
        ...
        bestClassifier = None
        bestParam = None
        bestClippedDiagnostic = None
        bestUnclippedDiagnostic = None

        for result in results:
            param = result['param']
            classifier = result['classifier']
            predictionError = result['perf']
            clippedDiagnostic = result['clippedDiagnostic']
            unclippedDiagnostic = result['unclippedDiagnostic']
            ...
            if (bestPerformance is None) or (bestPerformance > predictionError):
                ...
                bestParam = param
                bestClippedDiagnostic = clippedDiagnostic
...
class CRF(Skylines):
    ...
    def generateModel(self, param, reportValidationResult):
        ...
        diagnostic = 1.0
        return predictionError, regressors, diagnostic, diagnostic
...
class PRMWrapper(Skylines):
    ...
    def generateModel(self, param, reportValidationResult):
        ...
        clippedDiagnostic, unclippedDiagnostic = predictor.fit(self.smart_start)
